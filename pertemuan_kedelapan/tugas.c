...
int main()
{
    ...
    fprintf(file, "Nomor_TRX,Nama_Pasien,Umur,Alamat,Biaya Pemeriksaan,Jumlah_layanan,Total_biaya,Layanan_dipilih");
    ...
    do
    {
        ...
        fprintf(file, "\n%d,%s,%d,%s,%.2f,%d,%.2f,", nama, umur, alamat, totalHarga, layananCount, totalHarga);
        for (int i = 0; i < layananCount; i++)
        {
            fprintf(file, "%s%s", layananDipilih[i], (i == layananCount - 1) ? "" : ", ");
        }
